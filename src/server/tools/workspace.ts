...
import type { AppSettings, WorkspaceFile } from '../../shared/types.ts';
import { detectToolchains, platformName, runCommand, shellName } from '../services/commands.ts';
...
export function workspaceGuidance(files: WorkspaceFile[], commands: boolean): string {
...
function describeCommand(settings: AppSettings, toolchains: string[]): string {
  const { shell, commandTimeoutSeconds, maxCommandTimeoutSeconds } = settings.workspace;
  return (
    `Run a shell command on the user's computer (${platformName()}, ${shellName(shell)}) in the chat's workspace folder, and get back ` +
    'its exit code, output and the files it created or changed. Use it to compile, run, test or format the code in the workspace. ' +
    `Found on the PATH: ${toolchains.length ? toolchains.join(', ') : 'no common compilers or runtimes'}. ` +
...
export function workspaceTools(workspaces: WorkspaceService): ToolSource {
  const listFiles: ToolDefinition = {
    ...
    run: (args, context) =>
      guarded(async () => {
        ...
        const files = (await workspaces.exists(context.conversationId)) ? await workspaces.files(context.conversationId, folder) : [];
  ...
  const readFile: ToolDefinition = {
    ...
    run: (args, context) =>
      guarded(async () => {
        const file = await workspaces.read(context.conversationId, String(args.path));
  ...
  const writeFile: ToolDefinition = {
    ...
    run: (args, context) =>
      guarded(async () => {
        const result = await workspaces.write(context.conversationId, String(args.path), String(args.content), { append: args.append === true });
  ...
  const editFile: ToolDefinition = {
    ...
    run: (args, context) =>
      guarded(async () => {
        const result = await workspaces.edit(context.conversationId, String(args.path), String(args.old_text), String(args.new_text), args.replace_all === true);
  ...
  const deleteFile: ToolDefinition = {
    ...
    run: (args, context) =>
      guarded(async () => {
        const result = await workspaces.remove(context.conversationId, String(args.path));
  ...
  const moveFile: ToolDefinition = {
    ...
    run: (args, context) =>
      guarded(async () => {
        const result = await workspaces.move(context.conversationId, String(args.from), String(args.to));
  ...
  const findInFiles: ToolDefinition = {
    ...
    run: (args, context) =>
      guarded(async () => {
        if (!(await workspaces.exists(context.conversationId))) return ok('The workspace is empty.');
        const { matches, truncated } = await workspaces.search(context.conversationId, String(args.query), optionalString(args.path));
  ...
  function runCommandTool(settings: AppSettings, toolchains: string[]): ToolDefinition {
    return {
      group: 'commands',
      label: 'Run a command',
      defaultPolicy: 'ask',
      spec: {
        name: 'run_command',
        description: describeCommand(settings, toolchains),
        parameters: {
          type: 'object',
          properties: {
      ...
      run: (args, context) =>
        guarded(async () => {
          ...
          const dir = await workspaces.ensure(conversationId);
          const cwd = await workspaces.resolve(conversationId, optionalString(args.cwd), { allowRoot: true });
          const before = await workspaces.snapshot(conversationId);
          ...
          const after = await workspaces.snapshot(conversationId);
          ...
          const used = await workspaces.usage(conversationId);
  ...
  return {
    ...
    tools: async (settings) => [listFiles, readFile, writeFile, editFile, deleteFile, moveFile, findInFiles, runCommandTool(settings, await detectToolchains())],
  };
}
